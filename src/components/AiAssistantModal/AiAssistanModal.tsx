...
export const AiAssistantModal: React.FC<AiAssistantModalProps> = ({
    ...
    const [strategy, setStrategy] = useState<'balanced' | 'aggressive'>(
        'balanced',
    )
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setCsoData(null)
            const isEditor =
                (window.__SPNS__appConfig?.isEditor || 'false') === 'true'
            fetchGovData(isEditor, settings.country).then((data) => {
                setGovData(data)
            })
        }
        ...
        if (govData) {
            fetchCsoData(settings.country).then((data) => {
                ...
                const currentMonthlyContribution =
                    govData.revenue.pensionContributionsYTD / 12
    ...
    const newProjection =
        csoData && govData
            ? projectPensionGrowth(
            govData.revenue.pensionContributionsYTD / 12 + contributionIncrease,
            csoData.inflationRate,
            csoData.wageGrowthRate,
            investmentStrategies[strategy].rate,
        ) + (includeDormant ? govData.dormantPension.value : 0)
            : 0
                ...
                >
                    ...
                    {!isDataLoading && currentStep === 2 && selectedGoal && (
                        <Step3CloseTheGap
                            ...
                            setIncludeDormant={setIncludeDormant}
                    ...
                    {!isDataLoading && currentStep === 3 && selectedGoal && (
                        <Step4FinalPlan
                            planClosesAmount={planClosesAmount}
                            shortfall={shortfall}
                            onNext={() => paginate(1)}
                        />
                    )}
                    {!isDataLoading && currentStep === 4 && (
                        <Step5ActionPlan
                            plan={{contributionIncrease, strategy, includeDormant}}
